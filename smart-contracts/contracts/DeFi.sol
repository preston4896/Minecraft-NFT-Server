...
contract DeFi is ERC1155Holder {
  ...
  function calculateReturnAmount(uint256 time_in_seconds, uint256 amount, uint256 apy) public pure returns(uint256) {
    ...
    return (amount * (uint256(3) ** ((time_in_seconds/31556952) * apy)));
  }
  ...
  function lendToTrade(uint256 trade_id) public returns (bool) {
    ...
    tokensContract.safeTransferFrom(msg.sender, trades[trade_id].borrower, 0, trades[trade_id].borrowing_amount, "0x0");
  ...
  function payInterest(uint256 trade_id, uint256 amount) public returns (bool) {
    ...
    if (trades[trade_id].paid_back_amount >= trades[trade_id].borrowing_amount) {
      ...
      //   * trades[trade_id].apy));
      uint256 amount_to_be_paid = calculateReturnAmount(seconds_since_start, trades[trade_id].borrowing_amount, trades[trade_id].apy);
  ...
  function liquidateTrade(uint256 trade_id) public returns (bool) {
    ...
    uint256 amount_to_be_paid = calculateReturnAmount(seconds_since_start, trades[trade_id].borrowing_amount, trades[trade_id].apy) - trades[trade_id].paid_back_amount;
